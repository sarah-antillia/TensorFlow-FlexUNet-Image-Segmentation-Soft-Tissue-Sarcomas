...
class ImageMaskDatasetGenerator:
  ...
  def generate(self, h5_filepath, output_images_dir, output_masks_dir):
    self.output_images_dir = output_images_dir
    self.output_masks_dir  = output_masks_dir

    with h5py.File(h5_filepath, 'r') as data:
        print(data.keys())
        ct_data     = data["ct_data"]
        pet_data    = data["pet_data"]
        label_data  = data["label_data"]
        self.generat_mask_file(label_data,     output_masks_dir)
        self.generate_image_file(ct_data, pet_data, output_images_dir)

  # Generate sliced images by combining ct_slices and pet_slices 
  def generate_image_file(self, ct, pet, output_images_dir):
    for key in ct.keys():
      # Get ct and pet volumes from the key
      cv = ct[key]
      pv = pet[key]
      ...
      num  = min(nc, np)
      for i in range(num):
        # Get slices of ct and pet.
        cs = cv[i,:,:]
        ps = pv[i,:,:]

        # Normalize ct and pet solices.
        n_ct  = self.normalize(cs)
        n_pet = self.normalize(ps)
        
        filename = str(key) + "_" + str(i) + ".png"
        output_filepath = os.path.join(output_images_dir, filename)

        # Combine image ct and pet images.
        image = n_ct + n_pet
  ...
  def colorize_mask(self, mask):
    ...
    rgb_colors = [(255, 255, 0), (255, 255, 0),(0,255, 0), (0,0, 255)] 
    for i in range(3):
      (r, g, b)= rgb_colors[i]

      colorized[np.equal(mask, i+1)] = (b, g, r)
    return colorized
  
  def generat_mask_file(self, label, output_masks_dir):
    for key in label.keys():
      ...
      for i in range(n):
        ...
        if image.any() >0:
          ...
          print("Saved {}".format(output_filepath))
          if self.augmentation:
            self.augment(image, filename, output_masks_dir, border=(0, 0, 0), mask=False)
...
if __name__ == "__main__":
  try:
    h5_filepath = "./lab_petct_vox_5.00mm.h5"
    output_dir = "./Augmented-Soft-Tissue-Sarcomas-CTPET"
    ...
    generator.generate(h5_filepath, output_images_dir, output_masks_dir)
